...
- [x] **TestFlightワークフローを手動実行しビルドをアップロード** — 完了(1.0.1配信済みを2026-09-24にユーザー確認)。Actions → TestFlight → Run workflow(mainから)
- [x] **ASCで1.0.1バージョンを作成し提出** — 完了(配信済み・2026-09-24確認)。 `(+) バージョンまたはプラットフォーム` → 1.0.1 → **マーケティングURLに `https://ysu119190-lab.github.io/photouploader/`** → アップしたビルドを選択 → 「今回のアップデート内容」記入 → 審査に提出。※URLだけの変更は「ユーザー向け変更なし」と見られうるので小改善の同梱が無難
...
- [ ] **ASCで1.0.2を作成し提出**(あなた)— アップしたビルドを選択 → 「今回のアップデート内容」記入 → 審査に提出
  - **今回のアップデート内容(What's New)案**:
    ```
    ・バックアップした写真・動画を、撮影した月ごとのフォルダに保存するようにしました
    ・その他、軽微な改善を行いました
    ```
