...
- [ ] **AdMob本番化(残り・あなたの作業)**:
  ...
  - **⚠️ 未解決(2026-07-25): App Store商品ページに「デベロッパのWebサイト」リンクが無い＝マーケティングURLが空**。AdMobはiOSでこのマーケティングURLのルートドメイン直下の app-ads.txt を読むため、空のままだとクロールが確認済みにならない。**マーケティングURLは公開中バージョンではロックされ編集不可**(グレーアウト)。→ **次のアップデート(1.0.1等・編集可能バージョン)でマーケティングURLに `https://ysu119190-lab.github.io/photouploader/` を入力して公開 → その後AdMobで再クロール**、の順で対応する。app-ads.txtファイル配置とAdMobアプリ側設定は完了済みなので、URLさえ入れば通る。ブロッカーではない(未確認でも広告は配信・収益が少し絞られるだけ)
...
- [ ] **公開直後: AdMobをApp Storeリスティングに紐づけ → app-ads.txt の確認クロールを実行**(上記「AdMob本番化(残り)」の最後の項目。ストアURL確定が前提)。**ただしマーケティングURLが空でクロールが確認済みにならない問題あり(上記⚠️)→ 次のアップデートでマーケティングURLを入れてから再クロール。急ぎではない**
